    ...
    if request.method == "POST":
        ...
        if existing_account:
            ...
            try:
                ...
                db.session.commit()  # Cam kết thay đổi vào cơ sở dữ liệu
                flash(
                    "Facebook account added successfully!", "success"
                )  # Flash thông báo thành công
            except Exception as e:
                db.session.rollback()  # Nếu có lỗi, rollback để đảm bảo tính toàn vẹn dữ liệu
                flash(f"Error: {e}", "danger")  # Flash thông báo lỗi
...
def delete_fb_account(id):
    try:
        ...
        if account:
            ...
            db.session.commit()  # Cam kết thay đổi vào cơ sở dữ liệu
            flash(
                "Facebook account deleted successfully!", "success"
            )  # Flash thông báo thành công
        else:
            flash("Account not found.", "danger")  # Flash thông báo không tìm thấy
    except Exception as e:
        db.session.rollback()  # Nếu có lỗi, rollback để đảm bảo tính toàn vẹn dữ liệu
        flash(f"Error: {e}", "danger")  # Flash thông báo lỗi
...
def get_pages():
    ...
    if not access_token:
        flash(
            "Access token không được cung cấp.", "danger"
        )  # Flash thông báo thiếu access token
    ...
    try:
        ...
        if pages:
            flash(
                "Lấy thông tin các trang thành công", "success"
            )  # Flash thông báo thành công
            return redirect(url_for("pages.show_pages"))
        else:
            flash("Không thể lấy thông tin trang.", "danger")  # Flash thông báo lỗi
            error_message = "Không thể lấy thông tin trang."
            return render_template("error.html", error_message=error_message)
    except Exception as e:
        flash(f"Lỗi: {e}", "danger")  # Flash thông báo lỗi
